...
import { extractIdsFromText } from './ids.js';
const MD_LINK_RE = /\[([^\]]*)\]\(([^)]+)\)/g;
export function validateMdLinks(docsRoot, files) {
    const hits = [];
...
function resolveExisting(target) {
    ...
    if (fs.existsSync(path.join(target, 'index.md')))
        return true;
    if (fs.existsSync(target.replace(/\/$/, '') + '.md'))
        return true;
    return false;
}
export function depsFromFiles(files, selfId) {
    const deps = new Set();
...
export function dependentsOf(id, index, docsRoot) {
    const out = [];
