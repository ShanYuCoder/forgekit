...
import { AGENT_IDS, AGENT_LABEL, detectAgents, parseAgentTargets, } from './agents.js';
import { checkboxPrompt, promptLine, selectPrompt, } from './prompt.js';
const FE_ADAPTERS = ['nuxt4', 'nextjs', 'dotnet-line'];
const BE_ADAPTERS = ['fastapi', 'laravel', 'dotnet-integration', 'nestjs'];
...
function artifactgraphAvailable() {
...
export async function resolveInitWizard(opts) {
    ...
    // All lanes and adapters (including dotnet-line) need docsRoot to locate design specs.
    if (!docsRoot && opts.interactive) {
        const answer = await prompts.line('Docs hub path for CODEGENKIT_DOCS_ROOT (Enter to skip): ');
        if (answer)
            docsRoot = answer;
    }
    const installableOptional = artifactgraphAvailable() ? ['ArtifactGraph'] : [];
    const withOptional = opts.interactive && opts.requestedWith === undefined
