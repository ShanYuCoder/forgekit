...
import { packageRoot, packageVersion, resolveBeAdapter, resolveFeAdapter, resolveProjectRoot, } from './config/project-root.js';
...
import { BE_SKILLS, FE_SKILLS, harnessStatus, installHarness, pruneHarness, readInstallManifest, uninstallHarness, } from './install/harness.js';
...
import { runAdapterEngine, runCommonGen, runCssGen, runContractEngine } from './adapters/run.js';
...
function usage() {
  ...
  gen-common|gen-common:dry [--adapter=nuxt4|nextjs] [--docs-root=…] [--project-root=…] -- --surface=<surface> | --module=<CMP-*> [--id=…] [--json]
  gen-css|gen-css:dry [--adapter=nuxt4|nextjs|dotnet-line] [--docs-root=…] [--project-root=…] -- --in=… --out=…
...
function removeCli(dryRun) {
    ...
    const result = { removed: [], removable: [], preserved: [] };
    const targets = [
    ...
    for (const target of targets) {
        ...
        if (!dryRun) {
            try {
                ...
                result.preserved.push(`${target} (${error instanceof Error ? error.message : String(error)})`);
            }
        }
    }
    return result;
...
function runUninstallScope(scope, flags) {
    ...
    const doHarness = (root) => {
        ...
        if (result.manifestRemoved)
            console.log(`  manifest removed: ${result.manifest}`);
...
async function runUninstall(defaultScope) {
    ...
    if (requested && !UNINSTALL_SCOPES.includes(requested)) {
        throw new Error(`--scope must be one of: ${UNINSTALL_SCOPES.join(', ')}`);
    }
    const scope = defaultScope === 'repo' ? 'repo' : requested ?? 'all';
    if (process.stdin.isTTY && !flags.yes) {
        console.log(`Preview (${scope}):`);
        runUninstallScope(scope, { ...flags, yes: false });
        const accepted = await confirm(defaultScope === 'repo'
...
async function main() {
    ...
    if (command === 'init') {
        ...
        console.log(`Wired Codegenkit → ${agents.targets.join(', ') || '(none)'} (local)`);
        ...
        for (const file of harness.conflicts)
            console.log(`  conflict: ${file}`);
        for (const file of harness.stale)
            console.log(`  stale: ${file} (run codegenkit prune)`);
        if (harness.gitignore.length) {
            console.log(`gitignore: claimed ${harness.gitignore.map((entry) => entry.pattern).join(', ')}`);
        }
        for (const optional of selection.withOptional) {
            if (optional === 'ArtifactGraph') {
                console.log('  optional: ArtifactGraph selected — run `artifactgraph init --yes` in this repo to register it');
            }
        }
        if (selection.wireCodegraph) {
            ...
            if (codegraph.stdout)
                process.stdout.write(codegraph.stdout);
    ...
    if (command === 'prune') {
        ...
        for (const file of result.modified)
            console.log(`  keep modified: ${file}`);
        if (!yes && result.removable.length) {
            console.log('Dry-run only. Re-run with --yes to delete unmodified stale assets.');
        }
        console.log(`Prune: ${result.removed.length} removed, ${result.removable.length} removable, ${result.modified.length} modified kept`);
        return;
    }
    if (command === 'common-registry') {
        const result = validateCommonRegistry(root, arg('--registry'));
        console.log(`common.registry v${result.version}: OK (${result.entries} entries, ${result.aliases} aliases)`);
        console.log(`  path: ${result.path}`);
        return;
    }
    const docsRoot = arg('--docs-root');
    if (command === 'api-gen' || command === 'api-gen:dry') {
        printResult(runBeEngine({
    ...
    if (command === 'gen-common'
        || command === 'gen-common:dry'
        || command === 'common-gen'
        || command === 'common-gen:dry') {
        printResult(runCommonGen({
            adapter,
            projectRoot: root,
            docsRoot,
            argv: passthrough(command),
    ...
    if (command === 'gen-css' || command === 'gen-css:dry') {
        printResult(runCssGen({
            adapter,
            projectRoot: root,
            docsRoot,
            argv: passthrough(command),
