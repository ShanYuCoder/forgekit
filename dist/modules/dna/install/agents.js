...
function windowsUserHas(relative) {
    const usersRoot = '/mnt/c/Users';
    if (!existsSync(usersRoot))
        return false;
    try {
        return readdirSync(usersRoot).some((name) => {
            if (name.startsWith('.') || ['Public', 'Default', 'All Users'].includes(name))
                return false;
...
export function detectAgents(cwd = process.cwd()) {
    ...
    const found = [];
    const add = (id, detected) => {
        if (detected)
            found.push(id);
    };
    add('claude', existsSync(path.join(home, '.claude.json')) ||
        existsSync(path.join(home, '.claude')) ||
        existsSync(path.join(cwd, '.claude.json')) ||
        existsSync(path.join(cwd, '.mcp.json')));
    add('cursor', existsSync(path.join(home, '.cursor')) ||
    ...
    add('opencode', existsSync(path.join(xdgConfigHome(), 'opencode')) ||
    ...
    add('gemini', existsSync(path.join(home, '.gemini')) ||
        existsSync(path.join(cwd, '.gemini')) ||
        existsSync(path.join(cwd, 'GEMINI.md')));
    add('antigravity', existsSync(path.join(home, '.gemini', 'antigravity')) ||
    ...
    add('kilo', existsSync(path.join(home, '.kilocode')) ||
        existsSync(path.join(cwd, '.kilocode')) ||
        existsSync(path.join(cwd, '.kilo')));
    return found;
}
export function parseAgentTargets(raw, detected, fallback = ['cursor']) {
    ...
    for (const part of value.split(/[,\s]+/).filter(Boolean)) {
        ...
        if (!id) {
            throw new Error(`Unknown target "${part}". Known: ${agentIds.join(', ')}, agy, auto, all, none`);
        }
