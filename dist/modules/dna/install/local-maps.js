...
export const LOCAL_MAP_FILES = [PLATFORM_LOCAL_MAP, LEGACY_LOCAL_MAP];
const PLATFORM_SCHEMA = 'https://github.com/raintr91/platform-dna/blob/main/templates/schemas/platform-repos.schema.json';
const LEGACY_SCHEMA = 'https://github.com/raintr91/docskit/blob/main/templates/schemas/legacy-repos.schema.json';
function skeleton(schema) {
    return `${JSON.stringify({ $schema: schema, projects: {} }, null, 2)}\n`;
}
function projectCount(file) {
    try {
 ...
 */
export function mapKindForSystemId(systemId) {
    const id = systemId.trim();
    if (/^legacy-/i.test(id))
        return 'legacy';
    return 'platform';
}
export function localMapFileForSystemId(systemId) {
 ...
 */
export function ensureLocalRepoMaps(root) {
    ...
    const gitignoreEntries = LOCAL_MAP_FILES.map((pattern) => ({
        pattern,
        shared: true,
    }));
    return {
...
export function localMapsStatus(root) {
    const base = path.resolve(root);
...
//# sourceMappingURL=local-maps.js.map