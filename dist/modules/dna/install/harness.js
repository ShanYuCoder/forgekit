...
import { existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, rmdirSync, unlinkSync, statSync, writeFileSync, } from 'node:fs';
import path from 'node:path';
import { packageRoot, packageVersion, } from '../config/project-root.js';
import { forgetInstall, recordInstall } from './ledger.js';
import { canonicalGitignorePattern, ensureGitignoreEntries, removeGitignoreEntries, } from './gitignore.js';
import { mcpEntryHash, removeMcpServers } from './mcp-config.js';
import { isDnaConfigureRepoMapsSsot, isVendorThinConfigureRepoMaps, } from './configure-repo-maps.js';
import { localMapsStatus } from './local-maps.js';
import { agentDirs } from './agents.js';
...
function pruneEmptyDirs(root, file) {
...
export function harnessSourceToTarget(source, agentDir = '.cursor') {
...
const profileTypes = ['docs', 'fe', 'be', 'monolith', 'tests'];
...
function validateManifestFile(targetRel, value) {
    ...
    if (!isRecord(value))
        throw new Error(`Invalid manifest file record: ${relative}`);
    const source = normalizedRelative(String(value.source ?? ''), 'source path');
    if (!/^harness\/(?:common|docs|fe|be|tests|monolith)\/(?:adapters\/[^/]+\/)?.+/.test(source)) {
...
export function validateInstallManifest(value) {
    if (!isRecord(value))
        throw new Error('Invalid Platform DNA install manifest');
    ...
    if (!profileTypes.includes(value.type)) {
    ...
    if (value.maps !== undefined) {
        if (!isRecord(value.maps))
            throw new Error('Invalid Platform DNA install manifest maps');
        for (const [relative, record] of Object.entries(value.maps)) {
            if (!['platform-repos.json', 'platform-repos.example.json'].includes(relative) ||
                !isRecord(record) ||
...
function validCanonicalPattern(pattern) {
...
function validateManifestMcp(value) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value))
        throw new Error('Invalid Platform DNA install manifest mcp');
    ...
    if (!isRecord(value.servers))
        throw new Error('Invalid Platform DNA install manifest mcp servers');
    const servers = {};
...
export function readInstallManifest(root) {
    ...
    try {
        ...
        throw new Error(`Invalid Platform DNA install manifest JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function fileStatus(root, relative, file) {
...
export function getHarnessStatus(root) {
    const targetRoot = path.resolve(root);
...
export function pruneHarness(opts) {
    ...
    if (!manifest)
        throw new Error(`Platform DNA install manifest not found: ${manifestFile(targetRoot)}`);
    const statuses = Object.entries(manifest.files).map(([relative, file]) => fileStatus(targetRoot, relative, file));
    ...
    writeFileSync(targetPath(targetRoot, '.platform-dna/install-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`);
    return result;
}
function mergeGitignore(previous, next) {
...
export function installHarness(opts) {
    ...
    if (opts.type === 'fe' && (opts.feAdapter || opts.adapter)) {
        ...
        if (opts.feAdapter)
            overlaySpecs.push({ lane: 'fe', adapter: opts.feAdapter });
        if (opts.beAdapter)
            overlaySpecs.push({ lane: 'be', adapter: opts.beAdapter });
    }
    for (const spec of overlaySpecs) {
        const overlay = path.join(packageRoot(), 'harness', spec.lane, 'adapters', spec.adapter);
        if (existsSync(overlay))
            roots.push({ dir: overlay });
    }
    const result = {
        written: [],
        unchanged: [],
    ...
    const files = Object.fromEntries(Object.entries(previous?.files ?? {}).map(([relative, file]) => [
        relative,
        { ...file, state: 'stale' },
    ]));
    ...
    const agentGitignoreEntries = agentIgnorePatterns.map((pattern) => ({
        pattern,
        shared: true,
    }));
            ...
            for (const agentDir of targetDirs) {
                ...
                if (existsSync(target)) {
                    ...
                    if (!lstatSync(target).isFile()) {
                        ...
                        const replaceConfigure = configureRel &&
    ...
    const manifest = {
        ...
        maps: opts.seededMaps
            ? Object.fromEntries(opts.seededMaps
                .filter((map) => map.created || previous?.maps?.[map.path])
                .map((map) => [map.path, { sha256: map.sha256 }]))
            : previous?.maps,
    };
    const gitignore = mergeGitignore(previous?.gitignore, mergeGitignore(opts.gitignoreEntries, agentGitignoreEntries));
    if (gitignore.length)
        manifest.gitignore = gitignore;
 ...
 */
export function recordManagedExtras(root, opts) {
    const targetRoot = path.resolve(root);
    const manifest = readInstallManifest(targetRoot);
    if (!manifest) {
        throw new Error(`Platform DNA install manifest not found: ${manifestFile(targetRoot)}`);
...
export function uninstallHarness(opts) {
    ...
    if (manifest.mcp) {
        ...
        const expected = Object.fromEntries(Object.entries(manifest.mcp.servers).map(([name, record]) => [name, record.sha256]));
