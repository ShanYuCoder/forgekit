...
import { getHarnessStatus, installHarness, pruneHarness, readInstallManifest, recordManagedExtras, uninstallHarness, } from './install/harness.js';
...
import { ensureGitignoreEntries } from './install/gitignore.js';
import { CODEGRAPH_MCP_FILE, wireCodegraph } from './install/codegraph.js';
function arg(name) {
    const equal = process.argv.find((value) => value.startsWith(`${name}=`));
    if (equal)
        return equal.slice(name.length + 1);
...
function cliTargets() {
    const installDir = path.resolve(process.env.PLATFORM_DNA_INSTALL_DIR ??
        path.join(os.homedir(), '.platform-dna', 'bootstrap'));
    const binDir = path.resolve(process.env.PLATFORM_DNA_BIN_DIR ?? path.join(os.homedir(), '.local', 'bin'));
    return [
        path.join(binDir, process.platform === 'win32' ? 'platform-dna.cmd' : 'platform-dna'),
        installDir,
    ];
}
function reportCodegraphPlan(wire) {
    for (const server of wire.plan.wire) {
        const merged = wire.merge?.added.includes(server.name)
            ? 'wired'
...
function runCodegraphWire() {
    ...
    if (wire.manifestMcp) {
        ...
        if (mcpIgnore.changed)
            console.log(`  wrote: ${mcpIgnore.file} (${CODEGRAPH_MCP_FILE})`);
...
function printRepoUninstall(root, yes) {
...
function runLifecycle(scope, yes) {
    if (scope === 'repo') {
    ...
    if (discover) {
        for (const root of discoverInstalls(discover))
            repos.add(root);
    }
    if (!repos.size)
        console.log('  (no registered repos — try --discover <dir>)');
    for (const root of repos)
        printRepoUninstall(root, yes);
...
async function runUninstall(scope) {
    ...
    if (interactive) {
        ...
        const confirmation = await selectPrompt({
            message: scope === 'repo'
...
async function main() {
    ...
    if (command === 'prune') {
        ...
        if (result.dryRun)
            console.log('platform-dna prune: dry-run (pass --yes to delete)');
        return;
    }
    ...
    const interactiveInit = command === 'init' &&
        !has('--yes') &&
        Boolean(process.stdin.isTTY && process.stdout.isTTY);
    const codegraphFilter = list('--codegraph-repos');
    const codegraphCandidateKeys = command === 'init'
        ? wireCodegraph({ root, filterKeys: codegraphFilter, dryRun: true }).plan.wire.map((server) => server.key)
        : [];
    ...
    for (const file of maps.written)
    ...
    if (wire)
        ...
        console.log(`  codegraph: skipped ${codegraphCandidateKeys.length} repo(s) — run \`platform-dna codegraph:wire\` later`);
    }
    const mcpManifest = wire?.manifestMcp;
    const gitignoreEntries = [...maps.gitignoreEntries];
    if (mcpManifest) {
