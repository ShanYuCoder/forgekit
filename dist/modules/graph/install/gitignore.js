 ...
 */
export function stripLegacyGitignoreBlock(root) {
...
export function mergeGitignoreEntries(previous, next) {
...
export function agentPathIgnorePattern(repoRelative) {
 ...
 */
export function desiredGitignorePatterns(input) {
    const exclusive = ['.artifactgraph/', '.docskit/'];
    if (input.wroteLexicon)
        exclusive.push('artifactgraph/');
    if (input.createdConfig)
        exclusive.push('artifactgraph.json');
    const shared = [];
 ...
 */
export function applyGeneratedGitignore(input) {
...
export function gitignoreEntryStatus(root, entries) {
    if (!entries.length)
        return [];
    const file = gitignorePath(root);
