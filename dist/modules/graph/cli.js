...
import { assertProjectManifestCompatible, installProjectAssets, normalizeInstallTypes, parseInstallTypes, pruneProjectAssets, projectInstallStatus, uninstallProjectAssets, } from './install/project.js';
...
async function runInitAgents(opts = {}) {
    ...
    try {
        ...
        if (arg('--print-config'))
            return;
        console.log(`Wired artifactgraph → ${result.targets.join(', ') || '(none)'} (${result.location})`);
        ...
        for (const key of ['created', 'updated', 'skipped', 'conflicts']) {
            if (project[key].length)
                console.log(`  ${key}: ${project[key].join(', ')}`);
        }
        console.log(`gitignore: ${project.gitignore.changed ? 'updated' : 'unchanged'} ${project.gitignore.file}`);
        if (project.gitignore.added.length) {
            console.log(`  added: ${project.gitignore.added.join(', ')}`);
        }
        if (!project.types.includes('docs')) {
            console.log('note: non-docs ArtifactGraph indexes this repo only; use CODEGENKIT_DOCS_ROOT/DOCSKIT_ROOT to reach the docs registry hub');
        }
...
function suggestedTypes(stack) {
    if (stack === 'nuxt4-nest' || stack === 'nextjs-nest')
        return ['fe', 'be'];
    if (stack === 'nuxt4' || stack === 'nextjs' || stack === 'dotnet-line')
        return ['fe'];
    if (stack === 'docs-c4')
        return ['docs'];
    if (stack === 'e2e-plans')
        return ['test'];
    if (['laravel', 'fastapi', 'dotnet-integration'].includes(stack))
        return ['be'];
    return ['common'];
}
async function resolveInitTypes(stack) {
    const requested = arg('--type');
    if (requested)
        return parseInstallTypes(requested);
...
async function runInitProject() {
    ...
    console.log(`Initialized ${result.root} (stack=${ctx.stack}, types=${result.types.join(',')})`);
}
...
function cliLayout() {
    const nativeWindowsDir = process.platform === 'win32' && process.env.LOCALAPPDATA
    ...
    for (const target of [
        ...
        if (target === installDir && realOrSelf(target) === current) {
            ...
            try {
                ...
                result.skipped.push(`${target} (${error instanceof Error ? error.message : String(error)})`);
            }
        }
    }
    return result;
...
function runUninstallScope(scope, flags) {
    ...
    const removeRepo = (repoRoot) => {
        ...
        for (const file of result.preservedUnsafe)
            console.log(`  preserve unsafe: ${file}`);
        for (const pattern of result.gitignorePreservedShared) {
            console.log(`  preserve shared ignore: ${pattern}`);
        }
    };
    const removeMcp = (location, cwd) => {
    ...
    const removeCliInstall = () => {
        const result = removeCli(!flags.yes);
        for (const file of result.wouldRemove)
    ...
    if (scope === 'mcp-global' || scope === 'all')
        removeMcp('global', root);
    if (scope === 'cli' || scope === 'all')
        removeCliInstall();
    if (scope === 'all') {
        if (flags.yes) {
            if (removeLedger())
...
async function main() {
    ...
    if (cmd === 'analyze' || cmd === 'gaps') {
        ...
        store.close();
        if (cmd === 'gaps') {
            console.log(JSON.stringify({ gaps: result.gaps, askUser: result.askUser, cloudPromptSlice: result.cloudPromptSlice }, null, 2));
        }
        else {
            console.log(JSON.stringify(result, null, 2));
        }
    ...
    if (cmd === 'gen') {
        console.error('deprecated: `artifactgraph gen` executes product commands; use `recommend-command` / `allowlist-check`, then the owning kit');
        const cfg = requireRepoConfig(ctx.root);
        const commandKey = arg('--command');
        if (!commandKey) {
            console.error('Missing --command');
...
function performRebuild(repoRoot, repoId) {
    ...
    console.log(`Rebuilt index for ${repoId}: files=${summary.files} shells=${summary.designShells} common=${summary.commonIds} unit=${summary.unitPatterns} e2e=${summary.e2eBundles} lexiconHints=${summary.registryTagHints ?? 0} testTypes=${summary.testTypes ?? 0}`);
    console.log(JSON.stringify(pathResolutionSummary(repoRoot, cfg), null, 2));
}
function injectPackageJsonScript(repoRoot) {
