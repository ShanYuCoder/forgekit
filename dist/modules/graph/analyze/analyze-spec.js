...
function hasPrefix(tags, prefix) {
    return tags.some((t) => t.replace(/\s+/g, ' ').startsWith(prefix));
}
/**
 * Core analyzer for a single spec file.
 */
export function analyzeSpecFile(repoRoot, cfg, specPath, store) {
    ...
    if (fe && (profile === 'list' || profile === 'create' || profile === 'edit')) {
        if (!hasPrefix(tags, '#shell:')) {
            const suggested = profile === 'list' ? '#shell: DataListPage' : '#shell: DataFormPage';
            gaps.push({
    ...
    if (fe) {
        for (const col of doc.ui?.columns ?? []) {
            if (col.key && !col.component) {
                ...
                askUser.push(`[GRILL-MARK] Column ${col.key}: A) local cell  B) ${tag}  C) defer`);
            }
        }
    }
    ...
    for (const t of tags) {
        ...
        if (t.includes('#missing_info')) {
            ...
            askUser.push(`[MISSING-INFO] Resolve ${t}: A) fill from member answer  B) keep empty + defer  C) promote common only after confirm`);
        }
    }
    ...
    if (rawText.includes('#missing_info') && !tags.some((t) => t.includes('#missing_info'))) {
        gaps.push({
            kind: 'missing-info',
            message: 'Spec body contains #missing_info — treat as open business gap; grill hard gate before inventing or overwriting SSOT',
            suggestedTag: '#missing_info',
            source: abs,
            severity: 'warn',
            confidence: 0.9,
        });
        askUser.push('[MISSING-INFO] Inline #missing_info found: A) promote into tags/marks + ask member  B) keep note  C) defer');
    }
    ...
    if (!isLegacy && (doc.codegen?.profile || tags.length > 0)) {
        askUser.push('[GRILL-CONFIRM] Blocks/tags đề xuất từ IR (không clone legacy). Member xác nhận đúng trước khi gen? (yes / edit / defer) — local only, không cloud');
        gaps.push({
            kind: 'missing-hashtag',
...
function formatCloudSlice(gaps, tags) {
    const lines = [
