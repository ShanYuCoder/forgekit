 ...
 */
export function grillCheck(input) {
    let base;
    if (input.specPath) {
    ...
    if (input.specPath && input.store) {
        const duplicateGaps = checkDuplicateRoutes(input.repoRoot, input.cfg, input.specPath, base.tags, input.store);
 ...
 */
function checkDuplicateRoutes(repoRoot, cfg, specPath, existingTags, store) {
    ...
    const conflicts = detectDuplicateRoutes(combined).filter((c) => c.sources.some((s) => s !== currentSourceFile) && c.sources.includes(currentSourceFile));
    return conflicts.map((conflict) => {
        const otherSources = conflict.sources.filter((s) => s !== currentSourceFile);
        return {
            kind: 'duplicate-api-route',
 ...
 */
export function recordGrillDecision(store, subject, choice, payload) {
    store.rememberDecision('grill-confirm', subject, { choice, ...payload });
}
//# sourceMappingURL=grill-check.js.map