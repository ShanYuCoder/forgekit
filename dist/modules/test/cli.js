...
import { chooseAgentTargets, installAgents, uninstallAgents, } from './install/agents.js';
import { installHarness, pruneHarness, SKILLS_BY_TYPE, statusHarness, uninstallHarness, } from './install/harness.js';
...
Advanced uninstall filters:
  ...
  cases:render|cases:check|cases:coverage|tests:publish [--project-root <path>] -- …engine args
...
function printStatus(status) {
    ...
    for (const issue of status.compatibility.issues)
        console.log(`  issue: ${issue}`);
    for (const bucket of ['healthy', 'missing', 'modified', 'stale']) {
        console.log(`${bucket}: ${status[bucket].length}`);
        for (const file of status[bucket])
            console.log(`  ${file}`);
    }
}
...
function runUninstallScope(scope, flags) {
    ...
    const removeRepo = (repo) => {
        ...
        for (const file of result.preservedProtected)
            console.log(`  preserve protected: ${file}`);
        for (const file of result.missing)
            console.log(`  already missing: ${file}`);
        if (result.manifestRemoved)
            console.log(`  manifest removed: ${result.manifest}`);
    };
    const removeMcp = (location, repo) => {
        if (location === 'local') {
            ...
            if (result.removed.length === 0)
                console.log('  MCP (local): no testkit entries');
            return;
        }
...
async function runUninstall(defaultScope) {
    ...
    if (defaultScope === 'all') {
        ...
        if (requested) {
            if (!UNINSTALL_SCOPES.includes(requested)) {
...
async function main() {
    ...
    if (command === 'init') {
        ...
        if (!['tests', 'fe'].includes(type))
            throw new Error('--type must be tests | fe');
    ...
    if (command === 'prune') {
        ...
        for (const file of result.preservedModified)
            console.log(`  preserved modified: ${file}`);
        for (const file of result.preservedProtected)
            console.log(`  preserved protected: ${file}`);
        for (const file of result.missing)
    ...
    const env = {};
    if (arg('--tests-root'))
        env.TESTKIT_TESTS_ROOT = arg('--tests-root');
    if (arg('--docs-root'))
        env.TESTKIT_DOCS_ROOT = arg('--docs-root');
    ...
    if (command === 'cases:coverage') {
        printResult(runEngine({ engineRel: ['cases', 'check-coverage.mjs'], projectRoot: root, argv: passthrough(command), env }));
    }
    if (command === 'tests:publish') {
        printResult(runEngine({ engineRel: ['cases', 'publish.mjs'], projectRoot: root, argv: passthrough(command), env }));
    }
    if (command === 'testcase:gen' || command === 'testcase:gen:dry' || command === 'testcase:gen:all') {
        const argv = passthrough(command);
        if (command.endsWith(':dry') && !argv.includes('--dry-run'))
            argv.push('--dry-run');
        if (command.endsWith(':all') && !argv.includes('--all'))
            argv.push('--all');
        printResult(runEngine({
