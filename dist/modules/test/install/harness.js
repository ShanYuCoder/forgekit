import { existsSync, mkdirSync, readFileSync, readdirSync, realpathSync, rmSync, rmdirSync, statSync, writeFileSync, cpSync, } from 'node:fs';
...
import { managedRepoStatus, removeManagedRepoFiles, syncManagedRepoFiles, } from './managed-files.js';
...
function currentFiles(type, targets = ['cursor']) {
    const sourceRoot = path.join(packageRoot(), 'harness', type);
    const files = {};
    const skillRoots = new Set();
    for (const agent of targets) {
        if (agent === 'cursor')
            skillRoots.add('.cursor');
        else if (agent === 'gemini' || agent === 'antigravity')
            skillRoots.add('.agents');
        else if (agent === 'codex')
            skillRoots.add('.codex');
        else if (agent === 'claude')
            skillRoots.add('.claude');
        else if (agent === 'hermes')
            skillRoots.add('.hermes');
        else if (agent === 'opencode')
            skillRoots.add('.opencode');
        else if (agent === 'kiro')
            skillRoots.add('.kiro');
        else if (agent === 'kilo')
            skillRoots.add('.kilocode');
    }
...
function managedTarget(root, targetRel) {
...
function safeManagedTarget(root, targetRel) {
...
function isArtifactGraphOwned(targetRel) {
    return targetRel
        .toLowerCase()
        .split(/[\\/]/)
...
function compatibility(manifest) {
    ...
    if (manifest.schemaVersion !== 1)
        issues.push(`unsupported schemaVersion: ${String(manifest.schemaVersion)}`);
    if (manifest.package !== '@platform/testkit')
        issues.push(`unexpected package: ${String(manifest.package)}`);
    if (manifest.toolApi !== 1)
        issues.push(`unsupported toolApi: ${String(manifest.toolApi)}`);
    if (manifest.harnessApi !== 1)
        issues.push(`unsupported harnessApi: ${String(manifest.harnessApi)}`);
    if (!['tests', 'fe'].includes(manifest.type))
        issues.push(`unsupported type: ${String(manifest.type)}`);
    if (!manifest.files || typeof manifest.files !== 'object' || Array.isArray(manifest.files)) {
        issues.push('invalid files inventory');
    }
    if (manifest.managed !== undefined
    ...
    for (const [targetRel, metadata] of Object.entries(manifest.files ?? {})) {
        if (!managedTarget('/testkit-project', targetRel))
            issues.push(`unsafe managed target: ${targetRel}`);
        if (!metadata
...
export function installHarness(opts) {
    ...
    const result = { written: [], unchanged: [], conflicts: [], skipped: [] };
    ...
    writeFileSync(manifestFile(root), `${JSON.stringify({
        schemaVersion: 1,
...
export function pruneHarness(opts) {
    ...
    const manifest = loadManifest(root);
    if (!manifest)
        throw new Error(`Testkit install manifest not found: ${manifestFile(root)}`);
    ...
    for (const [targetRel, metadata] of Object.entries(manifest.files)) {
        if (!metadata.stale || currentTargets.has(targetRel))
            continue;
...
export function uninstallHarness(opts) {
    ...
    for (const [targetRel, metadata] of Object.entries(manifest.files)) {
        ...
        if (result.dryRun)
            result.wouldDelete.push(targetRel);
        else {
            rmSync(target);
            result.deleted.push(targetRel);
        }
    ...
    if (!result.dryRun && manifest?.type === 'tests') {
        removeVitepressScripts(root);
    }
    forgetInstall(root);
    pruneEmptyDirs(root, [...result.deleted, INSTALL_MANIFEST_PATH].map((relative) => path.join(root, relative)));
    return result;
}
function injectVitepressScripts(root) {
    ...
    try {
        ...
        if (!pkg.scripts)
            pkg.scripts = {};
        if (!pkg.scripts['tests:publish']) {
            pkg.scripts['tests:publish'] = 'testkit tests:publish';
            changed = true;
        }
        if (!pkg.scripts['docs:dev']) {
            pkg.scripts['docs:dev'] = 'vitepress dev';
            changed = true;
        }
        if (!pkg.scripts['docs:build']) {
            pkg.scripts['docs:build'] = 'vitepress build';
            changed = true;
        }
        if (changed) {
...
function removeVitepressScripts(root) {
    const pkgPath = path.join(root, 'package.json');
    if (!existsSync(pkgPath))
        return;
    try {
        ...
        if (pkg.scripts) {
            if (pkg.scripts['tests:publish']) {
                delete pkg.scripts['tests:publish'];
                changed = true;
            }
            if (pkg.scripts['docs:dev']) {
