...
export function contextOrphanSchemaBlock() {
    return [
...
export function parseContextOrphansDoc(doc) {
    ...
    const list = Array.isArray(root.contextOrphans)
        ...
        : Array.isArray(root.orphans)
            ? root.orphans
            : [];
    return list.map((item, i) => normalizeOrphan(item, i)).filter(Boolean);
}
export function scanModuleContextOrphans(repoRoot, moduleDir) {
...
function normalizeOrphan(raw, index) {
    ...
    const usesData = toStrList(action.usesData ?? action.data ?? action.operatesOn ?? action.entity ?? action.target);
    if (!hostSurface || !actionId || !screenData.length || !usesData.length)
        return null;
    if (isSubset(usesData, screenData))
        return null;
    return {
        ...
        severity: o.severity === 'info' || o.severity === 'warn' || o.severity === 'error'
            ? o.severity
            : 'warn',
...
function orphansFromDoc(doc, file) {
    ...
    for (const a of extractActions(doc)) {
        if (!a.usesData.length)
            continue;
        if (isSubset(a.usesData, screenData))
            continue;
        out.push({
            ...
            screenData,
            action: a,
            reason: `Action uses [${a.usesData.join(',')}] but screen displays [${screenData.join(',')}]`,
            severity: 'warn',
            source: file,
        });
    }
...
function screenDataFromDoc(doc, file) {
    const explicit = toStrList(doc.legacy?.screenData ??
        doc.review?.screenData);
    if (explicit.length)
        return explicit;
    const names = [];
    const spec = doc.spec;
    ...
    for (const raw of Array.isArray(spec?.relationships) ? spec.relationships : []) {
        ...
        for (const k of ['to', 'target', 'entity', 'child', 'name']) {
            if (r[k])
                names.push(String(r[k]));
        }
    }
    const gen = doc.gen;
    const codegen = gen?.codegen;
    if (codegen?.entity)
        names.push(String(codegen.entity));
    if (!names.length) {
...
function extractActions(doc) {
    ...
    if (Array.isArray(design?.actions))
        buckets.push(...design.actions);
    const legacy = doc.legacy;
    if (Array.isArray(legacy?.ui))
        buckets.push(...legacy.ui);
    if (Array.isArray(legacy?.behaviors))
        buckets.push(...legacy.behaviors);
    const out = [];
...
//# sourceMappingURL=context-orphan.js.map