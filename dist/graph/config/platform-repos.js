...
function readWorkspacePathFile() {
    const candidates = [
 ...
 */
export function resolveWorkspaceRoot(mapWorkspaceRoot) {
 ...
 */
export function loadPlatformReposMap(mapPath) {
    if (!mapPath) {
        throw new Error('ArtifactGraph no longer ships platform-repos.json; bind to the current product repo or pass an explicit legacy map path');
    }
    const file = mapPath;
...
export function resolveProject(projectId, mapPath) {
    ...
    if (!existsSync(p.root)) {
        throw new Error(`Project root missing: ${p.root} (${projectId}). Set ARTIFACTGRAPH_WORKSPACE to your bases folder (e.g. ~/workspace).`);
    }
...
export function detectStack(repoRoot) {
    if (existsSync(path.join(repoRoot, 'nuxt.config.ts')))
        return 'nuxt4';
    if (existsSync(path.join(repoRoot, 'next.config.ts')) || existsSync(path.join(repoRoot, 'next.config.js'))) {
        if (existsSync(path.join(repoRoot, 'nestgen')) || existsSync(path.join(repoRoot, 'server')))
            return 'nextjs-nest';
        return 'nextjs';
    }
    if (existsSync(path.join(repoRoot, 'pyproject.toml')))
        return 'fastapi';
    if (existsSync(path.join(repoRoot, 'artisan')) || existsSync(path.join(repoRoot, 'src', 'make_help.md')))
        return 'laravel';
    if (existsSync(path.join(repoRoot, 'Integration.sln')))
        return 'dotnet-integration';
    if (existsSync(path.join(repoRoot, 'Line.sln')))
        return 'dotnet-line';
    return 'generic';
}
//# sourceMappingURL=platform-repos.js.map