...
def resolve_spec_paths(spec: str | None, id_val: str | None) -> list[str]:
  ...
  import subprocess
  script = TOOLS_DIR.parents[1] / "shared" / "resolve-cli.mjs"
  try:
    res = subprocess.run(
    ...
    if not data.get("success"):
      print(f"fast-unit-gen: FAIL --id {id_val}: {data.get('error')}", file=sys.stderr)
      sys.exit(1)
    ...
    print(f"fast-unit-gen: --id {id_val} → {len(paths)} spec(s) ({kind})")
    return paths
...
def main() -> None:
  ...
  for spec_path in paths:
    try:
      _run(spec_path, dry_run=args.command == "dry", force=getattr(args, "force", False))
      if len(paths) > 1:
        print("")
    except Exception as e:
      failed += 1
      print(f"fast-unit-gen: FAIL {spec_path}: {e}", file=sys.stderr)
