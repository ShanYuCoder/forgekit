...
function parseArgs(argv) {
  const options = { dryRun: false, writeSpec: false, spec: null, execute: true, force: false, id: null }
  ...
  if (!options.spec && !options.id) {
    throw new Error(
      'Usage: pnpm api:gen --spec …/api/<seq>/01-backend-spec.yaml OR --id CMP-XXX'
    )
  }
...
async function resolveSpecPaths(options) {
  if (options.id) {
    const resolved = resolveHubId(laravelRoot, options.id, 'api-codegen')
    ...
    console.log(`api-gen: --id ${options.id} → ${resolved.paths.length} spec(s) (${resolved.kind})`)
...
async function generateOne(options, registry, specPath) {
  ...
  if (spec.i18n) {
    const localesDir = path.join(laravelRoot, 'lang')
    const i18nFiles = mergeI18nFlat(spec.i18n, localesDir, options.dryRun)
    for (const f of i18nFiles) {
      console.log(`  ${options.dryRun ? '[dry] ' : 'write'}: ${path.relative(laravelRoot, f.path)} (+${f.keysAdded} keys)`)
    }
  }
...
async function main() {
  ...
  if (specPaths.length > 1) {
    console.log(`api-gen: ${specPaths.length} spec(s) to process`)
  }
