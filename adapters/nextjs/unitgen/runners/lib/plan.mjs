...
import { isFormPageProfile } from '../../../../shared/fe-layout-profiles.mjs'
import {
  expandTagTemplate,
  getPattern,
  resolveOutputPath
...
async function appendSchemaPlan(ctx, registry, files, needsUnit, skippedPatterns, root) {
  ...
  if (!pattern) return
  if (ctx.profile !== 'list' && ctx.profile !== 'admin-crud') return
...
async function appendValidationPlan(ctx, registry, files, needsUnit, skippedPatterns, root) {
  ...
  const pattern = registry.patterns?.[patternId]
  if (!pattern) return
  if (!isFormPageProfile(ctx.profile)) return
...
async function appendComposableFormPlan(ctx, registry, files, needsUnit, skippedPatterns, root) {
  ...
  const hookPath =
    findManifestLayerPath(ctx.codegenManifest, 'hook', { pathIncludes: `use${ctx.entityPascal}Form` }) ??
...
async function appendServiceCreatePlan(ctx, registry, files, needsUnit, skippedPatterns, root) {
  ...
  const servicePath =
    findManifestLayerPath(ctx.codegenManifest, 'service', { pathIncludes: 'Form.service' }) ??
    webServicePath(`${ctx.entity}Form.service.ts`)
  const absoluteService = path.join(root, servicePath)
  ...
  const relativePath = resolveOutputPath(
    pattern.output ?? `tests/unit/services/${ctx.entity}Form.service.create.test.ts`,
    ctx
  )
