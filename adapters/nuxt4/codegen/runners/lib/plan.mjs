...
export function buildFilePlan(ctx) {
  ...
  if (profile === 'list') {
    ...
    add('composable', `composables/${entity}/use${entityPascal}Columns.ts`, 'list/useListColumns.ts.hbs')
    add('composable', `composables/${entity}/use${entityPascal}Filters.ts`, 'list/useListFilters.ts.hbs')
    if (ctx.hasExportBlock) {
      add('composable', `composables/${entity}/use${entityPascal}Export.ts`, 'list/useListExport.ts.hbs')
    }
  ...
  if (isFormPageProfile(profile) || ctx.formFields?.length > 0) {
    ...
    add('models', `models/${entity}/index.ts`, 'list/model.index.ts.hbs')
    add('service', `services/${entity}Form.service.ts`, 'create/service.ts.hbs')
    ...
    add('component', `components/${entity}/${entityPascal}Form.vue`, 'create/form.vue.hbs')
  }
  ...
  if (hasStoreTag) {
    add('store', `stores/use${entityPascal}Store.ts`, 'store/store.ts.hbs')
  }
