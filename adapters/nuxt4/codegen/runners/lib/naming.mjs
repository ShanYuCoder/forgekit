...
export function routeToPagePath(routePath, profile) {
  const layout = feLayoutForProfile(profile)
  const trimmed = String(routePath ?? '').replace(/^\//, '').replace(/\/$/, '')
  if (layout === 'root') {
    return String(profile).trim() === 'not-found' ? 'pages/not-found.vue' : 'pages/error.vue'
  }
  if (layout === 'dashboard') {
    return trimmed ? `pages/${trimmed}/index.vue` : 'pages/index.vue'
  }
  const group = layout
  if (!trimmed) return `pages/(${group})/index.vue`
  return `pages/(${group})/${trimmed}/index.vue`
}
